...
public class CourseDAL implements BaseDAL<Course> {
	...
	public Course getByCode(String code) {
		...
		Course course = null;
		try {
			PreparedStatement prs = (PreparedStatement) conn.prepareStatement(Constants.Course.GET_BY_CODE_COURSE);
	...
	public int insert(Course object) {
		...
		int re = 0;
		try {
			PreparedStatement prs = (PreparedStatement) conn.prepareStatement(Constants.Course.INSERT_COURSE);
	...
	public int udpate(Course object) {
		...
		int re = 0;
		try {
			PreparedStatement prs = (PreparedStatement) conn.prepareStatement(Constants.Course.UPDATE_COURSE);
	...
	public int delete(Course object) {
		...
		int re = 0;
		try {
			PreparedStatement prs = (PreparedStatement) conn.prepareStatement(Constants.Course.DELETE_COURSE);
			prs.setString(1, object.getCourseCode());
			re = prs.executeUpdate();
			prs.close();
			conn.close();
	...
	public List<Course> fillter(String sequnceFillter) {
		...
		try {
			...
			ResultSet rs = stam.executeQuery(Constants.Course.FILLTER_COURSE);
			while (rs.next()) {
				Course course = new Course();
				course.setCourseCode(rs.getString(Constants.Course.COURSE_CODE));
				course.setCourseCode(rs.getString(Constants.Course.COURSE_NAME));
				listCourse.add(course);
				stam.close();
				conn.close();
			}
